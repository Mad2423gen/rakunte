...
# ===========================================================================
def process_file(csv_file, csv_dir, conf_dir, intervaltime):
...
def export_ex(output_ex_files_dir, intervaltime):
    ...
    with ProcessPoolExecutor() as executor:
        # files = [os.path.basename(file_path) for file_path in
        #          glob.glob(os.path.join(csv_dir, f'{intervaltime}_*.csv'))]
        for csv_file in [os.path.basename(file_path) for file_path
                         in glob.glob(os.path.join(csv_dir, f'{intervaltime}_*.csv'))]:
            executor.submit(process_file, csv_file, csv_dir, conf_dir, intervaltime)
    print('process termination')
...
# ===========================================================================
def process_thumb(ttl):
    return [ttl[0], ttl[1], ttl[2], ttl[3], ttl[4], ttl[5]]
